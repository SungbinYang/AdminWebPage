...
public class OrderDetailSample extends StudyApplicationTests {
    ...
    private void item(User user){
        ...
        OrderGroupPayment paymentType = OrderGroupPayment.BANK_TRANSFER;
        switch (s){
            case 1 :
                status = OrderGroupStatus.ORDERING;
                paymentType = OrderGroupPayment.BANK_TRANSFER;
                break;

            case 2 :
                status =OrderGroupStatus.COMPLETE;
                paymentType = OrderGroupPayment.CARD;
                break;

            case 3 :
                status = OrderGroupStatus.CONFIRM;
                paymentType = OrderGroupPayment.CHECK_CARD;
                break;
        }
        ...
        for(Item item : items){
            ...
            OrderDetailStatus orderDetailStatus = switch (random.nextInt(3) + 1) {
                case 1 -> OrderDetailStatus.ORDERING;
                case 2 -> OrderDetailStatus.COMPLETE;
                case 3 -> OrderDetailStatus.CONFIRM;
                default -> OrderDetailStatus.ORDERING;
            };
            ...
            OrderDetail orderDetail = OrderDetail.builder()
                    ...
                    .status(type.equals(OrderType.ALL) ? OrderDetailStatus.ORDERING :orderDetailStatus)
                    .build();
